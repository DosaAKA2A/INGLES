/* INGLES — rate limit.
   ...
   del worker porque ese binding cuenta POR CENTRO DE DATOS: un atacante
   repartido entre veinte colos consigue veinte veces el limite. Para pedir
   codigos y para activar licencias eso no sirve.
...
export async function barreVencidos(env) {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("DELETE FROM uso_ia   WHERE dia < date('now', '-45 days')"),
